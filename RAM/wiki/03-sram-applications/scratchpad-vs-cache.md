...
Scratchpad 和 cache 最容易被混淆，是因为两者表面上都像”离计算很近的一小块 SRAM”——就像自动挡和手动挡的车表面都是”能开的汽车”。如果只看介质，这个判断没错；但如果把系统语义也纳入，就会发现它们几乎站在两种相反的设计哲学上。cache 是自动挡：硬件替你决定什么时候换挡（哪些数据值得暂存），你只管踩油门。scratchpad 是手动挡：什么时候换挡、换几挡，完全由你自己控制。两者的根本差异，不在 cell，不在 bank，而在”谁管理局部性”。
...
这组差异立刻会反映到面积效率上。cache 要为 tag、比较器、替换状态、coherence 或维护逻辑付费，因此在相同总面积预算下，真正用于存放有效 payload data 的 SRAM 容量会更少。scratchpad 没有这些元数据和命中判断路径，因此在同面积下往往能提供更高的数据净容量，也更容易按业务需求定制 bank 划分和端口组织。常见误解是认为 scratchpad 只是”没有 tag 的简化 cache”——就像说手动挡只是”没有自动变速箱的简化版”。更准确的说法是，scratchpad 把 cache 的硬件管理成本转移给了软件控制与调度复杂度：硬件更简单了，但驾驶员的技术要求更高了。

性能行为上的差异更值得架构师关注。cache 优化的是”平均情况”——就像出租车司机凭经验选路线，大多数时候走得不错，偶尔踩雷堵在路上。scratchpad 优化的是”计划内情况”——更像按预定路线行驶的公交车，不会给你惊喜，但也不会给你意外。如果 workload 有较好的时间/空间局部性，cache 会让大量访问自动命中本地 SRAM；如果软件能提前看见数据流结构，scratchpad 就可以在计算前显式搬运、双缓冲、复用和排布数据，从而把局部性从”希望硬件猜到”变成”设计时就确定好”。因此，cache 更适合控制流复杂、访问模式难静态预测的通用负载；scratchpad 更适合访问模式规则、块化明显、可由编译器或运行时调度的数据流型负载。这里的差别不是谁绝对更先进，而是谁在当前 workload 上更接近正确的责任分配。
