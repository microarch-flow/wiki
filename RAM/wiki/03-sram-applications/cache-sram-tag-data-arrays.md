...
把 cache 粗略描述为“用 SRAM 做出来的高速缓存”当然没错，但这句话最大的问题是把 cache 的关键复杂性抹掉了。真正让 cache 成为 cache 的，不是它用了 SRAM，而是它把一块 SRAM 变成了一个由硬件管理的、带命中不确定性和替换语义的数据层。也就是说，register file、scratchpad 和 cache 都可能用 SRAM 实现，但只有 cache 会主动替软件猜：当前访问的地址有没有已经被我留在本地，如果有，它在哪一行、哪一路、是否有效、是否脏、是否需要回写。这个“猜”的过程，决定了 cache 绝不是一块单纯的数据阵列。

这也是为什么 cache 往往天然分成 `tag array` 和 `data array`。data array 负责存放真正的数据块，通常按 cache line 组织，因此容量占大头。tag array 则存放每条 line 对应的地址标签、valid/dirty 等元数据，并参与比较判断。两者虽然都常常由 SRAM cell 组成，但它们服务的时序任务不同：data array 关注的是在命中后尽快把整块或部分字返回；tag array 关注的是在访问的很早阶段尽快判定“这是不是我要的那条 line”。常见误解是认为 tag 只是 data 的附属注释；实际上，很多 cache 的关键频率路径首先卡在 tag 查找和命中判决，而不只是卡在数据本身的读取。
...
从 SRAM 角度看，cache 的特殊性还在于它把“不确定性”制度化了。register file 的访问语义是确定的，给定寄存器号就必须拿到值；scratchpad 的数据位置由软件显式决定；TCM 的地址映射固定可知。而 cache 的核心价值，恰恰来自允许硬件在局部容量里动态保留“也许接下来还会用到”的数据。于是 SRAM 在这里承载的不只是存储，还承载了一个基于局部性的投机策略。这种策略的收益是平均延迟下降，代价是引入命中/未命中不确定性、替换副作用和更复杂的控制路径。
