...
为什么 DRAM 的行激活开销远比 SRAM 大，而且必须在接口层显式拆成”先开行、后取列”两步。SRAM 同样有行列解码和共享 sense amp，但 6T cell 信号强且读操作非破坏性，行列开销差异不大，对外表现接近”给地址、出数据”；DRAM 的 1T1C cell 信号极弱且读是破坏性的，整行感测与恢复成为不可省略的高成本前置步骤。更准确地说，行列解码、位线预充和 sense amp 共享这几件事，是怎样一起把 DRAM 访问结构逼成”先开行，后取列”的。

## 正文

理解 DRAM 时，一个最值得先纠正的直觉是：它并不是“用得更慢一点的随机访问存储器”。从软件语义上看，DRAM 当然支持随机地址访问；但从阵列物理上看，它的随机访问不是直接落到单个 cell 上，而是必须绕过一个非常强的结构前提：单个 1T1C cell 太弱，弱到不值得也不可能为每个 bit 单独配一套始终可用的放大和恢复资源。因此，DRAM 的访问必须先经过整行级别的感测与恢复，再从已经被打开的那一行中选出所需列。打个比方：SRAM 像一个书架，虽然也要先找到正确的层（行）再找到正确的格子（列），但每本书本身就是硬封面精装的，拿起来毫不费力，而且拿走看完还能原样放回；DRAM 更像一个巨型冷库——每本书都是写在薄纸上的，你必须把整个货架区的灯全打开、用放大镜把整排薄纸上的字迹辨认并抄录一遍（因为一碰就会褪色），然后再从抄录好的副本中找到你要的那一页。这个“必须”不是协议选择，而是阵列经济性与物理可行性的结果。
...
可以用一个有限度的类比帮助理解：SRAM 和 DRAM 都需要先选行再选列，都要经过 sense amp，但 SRAM 更像是房间里的灯虽然也要开，但灯泡功率大、一拧就亮，而且开灯不会把房间里的东西弄乱；DRAM 更像是灯泡极暗，你必须先把整层的照明系统全部接上并调到最亮，而且照亮的过程本身会扰动房间里的物品，之后还得花时间把物品恢复原位。这个类比只适合帮助理解”为什么 DRAM 的行激活开销远大于 SRAM”，一旦进入 timing 和恢复细节，仍然要回到 wordline、bitline 和 sense amp 的精确语言。

列解码 `column decode` 的意义，恰恰建立在这一步之后。因为整行已经被 sense amp 打开，所以列选择不再是在巨大二维阵列里“随机抓取某个 cell”，而是在一行已经展开的位线结果中选择某一段输出。也就是说，DRAM 的 row/column 并不是两个对称层级。row 负责把弱模拟信息变成可用数字状态，并付出最大的一次性开销；column 负责在这个已打开状态上做相对便宜的局部多路选择。这就像去图书馆借书：row 相当于让管理员把整个书架从密集书库里推出来（很费力，耗时长），column 相当于从已经推出来的书架上挑一本（很轻松，几秒钟）。显然，如果你要连续借同一架上的好几本书，只需推一次书架；但如果每本书都在不同架上，你就得反复推进推出。这也是为什么 row miss 和 row hit 的成本会差那么多，后面 row buffer 概念才会成立。

为什么 DRAM 不能像 SRAM 那样让行激活开销变得几乎不可感知？答案还是 cell 结构和面积取舍。SRAM 的 sense amp 同样按列共享，但 6T cell 本身信号强、读非破坏性，sense amp 只需加速差分放大即可完成读取，不需要额外的恢复周期，所以行列开销差异很小，对外表现接近单周期访问。DRAM 如果想达到类似效果，就需要更大的 cell 电容或更多的局部放大资源来补偿 1T1C 的弱信号和破坏性读取，但这会显著膨胀面积，直接破坏 DRAM 的密度优势。也就是说，DRAM 之所以选择“整行感测 + 列选择”，本质上是在说：为了用极少的外围电路服务极大量的 cell，必须接受行级粒度的访问前置开销。
...
从工程取舍看，DRAM 的行列分工本质上是把访问代价做成高度不均匀：row 代价大而稀疏，column 代价小而频繁。这很像坐飞机出差——买机票、过安检、等登机（row activate）花的时间和精力远大于在飞机上换个座位看不同文件（column access）。如果你一趟出差能处理十件事，这张机票就很值；如果每件事都要飞一趟，效率就会极低。这样做的好处，是如果 workload 恰好在一行内有足够局部性，前面的开销可以被摊薄；坏处是如果访问模式频繁跨行，系统就会不断支付大额前置成本。这也是为什么后面 page policy、address mapping 和 memory controller 调度会变得如此重要，因为它们其实都在回答同一个问题：怎样让昂贵的 row 开销被更充分摊销。
