...
所以，设计 weight buffer 时，第一个问题从来不该是“芯片上还有多少 SRAM 可分”，而应该是：`当前 workload 中，权重的有效复用距离有多长`。如果一组权重只会在很短的窗口里被消费一次，那么把它常驻很久没有意义；如果一组权重会在同一 tile、同一 batch、同一 head 或同一输出通道块上反复使用，那么多留它一会儿，往往比再扩一点算力阵列更值钱。换句话说，weight buffer 的本质不是一块”权重仓库”，而是一个把外存读取次数换成本地驻留时间的装置。就像一个厨师做宴席——如果某种调料每道菜都要用，最明智的做法是从储藏室取一次放在灶台边，用完整场宴席再收回去，而不是每做一道菜就跑一趟储藏室。weight buffer 的价值，正在于让高频复用的权重”留在灶台上”。
...
这就把容量问题自然推到 `banking` 上。一个常见误解是，把 weight buffer 设计理解成”容量定完后再切 bank”。其实在很多 NPU 里，banking 先于总容量成为一等公民，因为它决定了权重能否按阵列所需形状被同时读取。这就像图书馆的书架设计——你不能只关心”总共能放多少本书”，更要关心”50 个读者同时来借不同分类的书时，是否每人都能走到独立的书架前而不用排队”。如果所有书都堆在一个大仓库里，总容量够了，但同时取书的效率反而比小书架更差。阵列如果是按列广播权重，bank 组织可能要优先匹配列分发；如果不同 PE cluster 需要并行读取不同通道块，bank 组织又要优先匹配 cluster 粒度。你最终不是在设计一块抽象 SRAM，而是在设计一个与下游消费拓扑对齐的“读出几何形状”。
...
这也说明了一个很常见的误区。有人会问：“模型都这么大了，片上几 MB 或几十 MB weight buffer 有什么用？”这个问题隐含的前提是，只有把整模型装进去才算有价值。实际上，weight buffer 的价值从来不是”装下全模型”，而是”装下当前最值得不回外存的那部分权重工作集”。就像出差时的随身行李箱——你不会把整个衣柜搬走，而是只带这几天最需要的衣物；行李箱的价值不在于能装下你所有的衣服，而在于让你在出差期间不用每天跑回家换衣服。只要这部分工作集能在合适的时间窗口内被高频复用，本地驻留就有巨大价值。反过来，如果某部分权重本来就只用一次，那你把它死撑在片上反而是在浪费面积。
