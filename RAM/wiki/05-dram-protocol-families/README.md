...
上一章已经把 DRAM 的物理和阵列逻辑搭起来了，这一章要回答的是：这些底层约束是如何一步步被接口化、标准化、产品化，最终变成系统能看见的命令、时序参数和不同协议家族。也就是说，这里不是另起一条新主线，而是在讲”物理债务如何被翻译成协议契约”。如果上一章是在解剖 DRAM 的身体结构，这一章就是在看它的”交通法规”——法规不是凭空立的，每一条都能追溯到某个物理路况限制。
...
从这里开始，协议家族之间的分化也会变得更有逻辑。`DDR`、`LPDDR`、`GDDR`、`HBM` 并不是四个彼此无关的名字，而是站在同一套 DRAM 物理基础上，针对不同系统目标重新布置 trade-off 的几条路线。通用服务器主存重视容量、生态和板级扩展，所以 DDR 路线会围绕通用性和通道扩展去平衡；移动设备更在意功耗和待机行为，所以 LPDDR 会对供电状态、I/O 摆幅和自刷新机制更敏感；图形和独立加速器更在意总带宽，于是 GDDR 会把单 pin 速率和更激进的接口预算往前推；HBM 则更进一步，把协议和堆叠/近距 I/O 绑在一起，换取超高带宽密度和更好的每 bit 能耗。就像同样是"汽车"，家用轿车、越野车、跑车和赛车都基于内燃机，但它们分别为舒适、通过性、速度和极限性能重新排布了同一套基础技术的 trade-off。
...
1. [ACT/RD/WR/PRE：DRAM 命令集为什么长这样](./commands-act-rd-wr-pre.md)
2. [关键 timing 参数：tRCD/tRP/tRAS/tCL 的物理来源](./timing-parameters-trcd-trp-tras.md)
3. [DDR：为什么要在时钟两边都传数据](./why-double-data-rate.md)
4. [DDR1 -> DDR5：每一代的核心改变是什么](./ddr-generation-evolution.md)
5. [LPDDR：为了功耗放弃了什么](./lpddr-low-power-mobile.md)
6. [GDDR：为带宽放弃了什么](./gddr-graphics-bandwidth.md)
7. [HBM：从协议层面看它和 DDR 的根本不同](./hbm-stacked-wide-io.md)
8. [四种 DRAM 协议的 trade-off 全景图](./ddr-lpddr-gddr-hbm-tradeoff-map.md)

如果你这次主要想补“DDR/LPDDR/GDDR/HBM 到底怎么分家”的直觉，优先看 3 -> 8。第一次系统阅读，建议完整按 1 -> 8 顺序走。
