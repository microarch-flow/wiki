...
你可以把整个过程想象成去银行保险柜取东西：先让工作人员打开保险柜（ACT，等 tRCD），然后找到你的格子取出物品（RD，等 tCL），取完后工作人员要锁好保险柜（PRE，等 tRP），你才能请求打开另一个保险柜。如果你要取的东西在同一个已打开的保险柜里，就省掉了开锁和关锁的时间——这就是 row hit 快的原因。
