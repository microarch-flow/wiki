...
第二层损失来自 `请求粒度与 burst 粒度不匹配`。DRAM 和控制器更喜欢长 burst、大块搬运，因为 activate 成本、列访问启动成本和数据总线占用开销都可以被摊薄。如果 workload 总是小而碎的请求，即使总字节数不少，也会不断重复支付前置开销。你可以把它理解成：峰值带宽假设的是“卡车一直满载跑”，而很多真实流量却是在不停发半空的车。小请求、随机请求和大量分散的 cache miss 往往会在这里掉很多效率。
...
第五层损失来自 `bank/channel 并行没有被真正利用`。理论峰值通常默认多个 bank、多个 channel 都能保持较高利用率。但现实中，地址映射也许把热点压在少数 bank 上，某些 master 的请求流也许本身不够宽，或者 queue depth 不够深，导致其他 bank/channel 明明空着却吃不到流量。此时系统看起来“总资源挺多”，但真正忙碌的只有一小部分。换句话说，并行资源是潜在带宽，不是自动到账的带宽。
...
这也解释了为什么某些系统升级了更高代 DDR 或更多 HBM stack 后，性能却没按比例提升。外层带宽天花板当然更高了，但如果之前主要卡在 bank hotspot、refresh 干扰、控制流阻塞或片上供数链路，那么新的峰值只是在更高的位置继续漏水。换句话说，扩大水塔高度不会自动修好漏管子。系统级性能优化之所以难，就难在很多问题并不在“总资源不够”，而在“资源兑现形状不对”。
