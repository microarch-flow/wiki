...
MCU、通用 SoC 和 AI 芯片就像**小镇、中等城市和超级大都市**——都需要交通系统（BUS），但角色完全不同。小镇里一条主路就是全部交通（MCU 的 BUS 是系统主骨架）；中等城市需要快速路+公交+人行道并存（SoC 的 BUS 同时承担控制面和部分数据面）；超级大都市的大部分货运靠高铁和地铁网络（NoC/HBM fabric），但市政管理、消防、邮政仍然靠普通道路系统（BUS 承担配置、状态、boot、debug）。

这页用同一组视角比较三类"城市"：共享强度、组织方式、事务语义、能力上限。目标不是贴标签，而是解释为什么不同规模的系统会把 BUS 推向不同形态。
...
小镇居民少、车不多，一条主路加几条支路就够用了。MCU 的 Resource 数量少，master 数量有限，外设多但流量小。BUS 设计的重点是低面积、低功耗、易验证和确定行为——就像小镇不需要立交桥和地铁，一个红绿灯管好就行。
