...
软件写下的都是 load/store——看起来一样的动作，但实际上是在和三种完全不同的”店铺”打交道：

- **普通内存**像**图书馆**——借书还书，书还在架上，你可以复印（cache），可以提前预约下一本（prefetch），偶尔调换还书顺序也无所谓。
- **MMIO**像**药房柜台**——你说要什么药，药剂师给你拿（read 可能触发 clear），你递上处方（write 可能启动设备），顺序不能乱，拿错了不能退。
- **APB 外设**是**药房柜台后面的慢速药库**——药剂师还要走进去翻找（bridge + PREADY wait），偶尔找不到会报错（PSLVERR），你在外面干等着。

这页说明：为什么不能把药房当图书馆用（寄存器当普通变量），不能把药库当”慢一点的图书馆”（APB 当慢内存），也不能用图书馆的规则管药房（用 cache/barrier 规则替代设备协议）。
