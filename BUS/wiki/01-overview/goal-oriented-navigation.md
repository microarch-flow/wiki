...
[学习路线图](./learning-roadmap.md) 是系统补课的完整教程——从第一章按顺序读到最后一章。本页更像一本**急救手册**：你已经知道自己哪里痛（RTL/协议、SoC 集成、DMA/Memory、调试定位或方案评审），需要用最短路径拿到可执行判断。

急救手册的价值是快，代价是可能漏掉病因。目标导向阅读一定会牺牲完整性：它减少无关细节，但容易漏掉前置概念。多数路径会保留必要基础文章；对调试和评审类路径，则必须依靠回跳规则补足前置概念，防止直接把症状当诊断、把 checklist 当结论。

## 目标 1：快速建立系统判断力（拿到城市规划的鸟瞰图）

当你面对一张 SoC 框图，想先判断”这里为什么用 BUS、哪里应该用 NoC、哪里只需要 point-to-point”时——就像看一座城市的规划图，先搞清楚哪里该修高速、哪里该修地铁、哪里只需要人行道。优先读问题定义和分类框架。不要先读 AXI 细节，因为此时你需要的是互连选型坐标，而不是某个协议的字段能力。
...
## 目标 2：做 RTL、协议适配或互连设计（造桥修路的施工手册）

当你要实现 master/slave、bridge、adapter、crossbar 或协议转换时，核心风险不是把螺丝型号记错，而是桥梁的承重、排水、伸缩缝没设计好——对应到 BUS 就是通道之间的依赖、outstanding 状态、返回匹配、回压传播和顺序语义没闭合。
...
## 目标 3：做 SoC 集成、bring-up 或系统软件（新楼入住的水电验收）

当问题发生在 boot、MMIO、debug、interrupt、cacheability 或 IOMMU 上时，你不关心高速公路能跑多快，而是关心"水通了没有、电接对了没有、门锁能不能用"。协议吞吐不是第一优先级。你更需要知道软件看到的地址、属性、状态和完成通知，如何映射到 BUS transaction。
...
## 目标 4：做 DMA / Memory / 性能分析（物流效率优化）

当目标是解释吞吐、尾延迟、DDR 抖动或 DMA 数据搬运效率时，你就像一个物流经理在追问"为什么理论运力够，实际送达率却不行"。阅读路径应该沿数据面走。不要只看理论带宽，因为真实瓶颈经常出现在 outstanding 深度（同时在途的包裹数上限）、burst 拆分（大件被拆成小件）、memory controller 调度（仓库分拣效率）和 return path（回程空车率）。
...
## 目标 5：做调试定位和故障复盘（交通事故现场勘查）

调试就像交通事故调查：先判断事故类型（追尾？侧翻？堵死？），再沿事故链回溯。直接看全部波形就像对着满屏监控录像来回翻，却不知道第一辆车是在哪个路口失控的。调试路径要先分类现象，再追事务生命周期。
...
## 目标 6：做汇报、方案评审或知识沉淀（给领导画架构图）

当你要向别人解释 BUS 方案，或评审一个互连设计，最重要的是把协议能力、系统角色、风险和观测点放在同一张判断表里——就像给市长汇报交通规划，不需要讲每个红绿灯的配时，而要用统一的指标（通行能力、事故率、成本）说话。此时不需要重读全部机制，而要用 reference 文档统一语言。
