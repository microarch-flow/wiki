...
debug path 是 BUS 系统里的侧向入口。它让外部调试器、片上 debug controller、DAP/access port 或 trace/debug master 在 CPU 未正常运行时仍能读写寄存器、访问内存、halt/resume CPU、采集现场。

它的特殊性不在于“debug 权限更高”，而在于它必须跨越正常软件路径之外的状态：CPU hang、boot 未完成、外设未初始化、低功耗、局部 reset、安全锁定。设计 debug path 时，核心问题是可达性、隔离、优先级和副作用如何同时成立。
