...
shared bus、bus matrix 和 crossbar 解决的是同一个问题：多个 master 和多个 slave 如何共享片上访问路径。它们的差别不是“新旧代际”，而是同一时间可建立的不冲突 transaction path 数量，以及阻塞会扩散到多大范围。

互连组织的演化动力来自并发需求。一个 CPU 访问少量外设时，shared bus 的简单性更有价值；CPU、DMA、display、debug 同时访问 SRAM、DDR 和 MMIO 时，全局串行化会把低速路径的等待传播到高速路径。bus matrix 和 crossbar 用面积、布线、仲裁和验证复杂度换取局部并发。
