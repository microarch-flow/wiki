...
bridge 不是把两边信号“接起来”的 glue logic。它是 BUS 系统里的语义转换边界：上游看到的 transaction 能力、时钟节奏、数据粒度、错误返回和 ordering 规则，到了下游可能被压缩、拆分、延迟或重写。

这页关注三个最常见的转换：协议转换、时钟域跨越和位宽适配。它们的共同风险是，上游协议允许的行为不等于下游路径能承接的行为。建模时必须把 bridge 当成一个有队列、有状态、有完成条件的组件，而不是零延迟连线。
...
CDC bridge 的目标不是“让信号过去”，而是让 transaction 在两个时钟域之间保持完整性。请求、数据、response、错误和 reset 状态都要跨域，任何一个方向的拥塞都可能影响另一个方向。
