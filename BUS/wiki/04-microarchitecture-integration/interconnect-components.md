...
片上 BUS 互连不是把几根线焊在一起——它更像一个**快递分拣中心**的内部结构：有收件柜台（input buffer）、地址分拣机（decoder）、路线规划员（route selection）、发件窗口排队系统（arbiter）、大小件转换区（adapter）、回执送达通道（return path）。

评估一个快递分拣中心，关键不是”有没有传送带”，而是每个包裹经过哪些环节、在哪个环节排队、哪个环节决定顺序、哪个环节堵了会把压力传回收件柜台。
...
Decoder 就像快递分拣中心的**自动分拣机**——扫描包裹上的地址，然后把它推到对应的出口通道。它不仅决定”这个包裹去哪”，还决定”地址不存在怎么退回”、”两个地址重叠怎么办”、”某些区域是否需要安检放行”。
...
Arbiter 就像分拣中心出口处的**红绿灯调度员**——三个方向的包裹都要走同一个出口，调度员决定谁先走。它把并发请求压成一个可执行顺序，因此它是延迟抖动、公平性和 QoS 的主要来源。
