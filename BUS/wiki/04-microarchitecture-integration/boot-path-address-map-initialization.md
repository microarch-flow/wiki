...
芯片上电就像**一栋大楼的早晨开门流程**：不是所有灯和电梯同时亮起来。保安先拿到钥匙开大门（reset vector → boot ROM），然后打开大厅灯和监控（always-on 寄存器），接着逐层启动电梯、空调、网络（SRAM、DRAM、外设、DMA）。

boot path 的核心问题是：在每个开门阶段，哪些楼层可以进入（哪些地址能访问），走哪条通道（经过哪条路径），进了没开门的楼层会怎样——是被告知"此楼层未开放"（返回错误），还是卡在门口永远等下去（hang）。
