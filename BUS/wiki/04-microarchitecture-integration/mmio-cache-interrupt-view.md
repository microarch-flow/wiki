...
软件看到的是同一条街道上的三种店铺：书店（memory，可以反复翻书不会变）、药房（MMIO，拿了药就从货架消失了）和消防报警器（interrupt，突然响起来你必须处理）。地址空间一样，但访问规则完全不同。

MMIO、cacheable memory 和 interrupt 分别代表三类软件可见语义：寄存器副作用、缓存一致性边界、异步事件通知。核心问题不是”地址能不能访问”，而是你去的是书店还是药房——能不能缓存、能不能乱序访问、读写有没有副作用。

## MMIO 不是慢一点的内存

MMIO 看起来和读写内存一样——都是 load/store——但它更像**按药房柜台的按钮**而不是翻书架上的书。读一次寄存器可能会清掉报警状态（你按了"确认"按钮，报警就消了），写一次寄存器可能启动 DMA（你按了"启动"按钮，机器就开始运转）。
