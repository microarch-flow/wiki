...
想象一个建筑综合体：一楼是高速物流仓库（需要叉车和传送带），二楼是办公区（需要走廊和门禁），三楼是档案室（只需要安静和一把钥匙）。如果把三层楼的需求都塞进同一个空间——叉车在档案室里跑，文员在传送带旁边办公——既危险又低效。

分层总线做的就是**给不同需求分楼层**。高性能主干（物流仓库）承担 burst、outstanding、宽数据和多 master 争用；外设子树（档案室）承担低速寄存器访问；bridge 是楼层之间的电梯和楼道，负责协议、时钟、位宽、错误和回压转换。
...
Bridge 就像建筑里的**货梯**——它不只是一个洞，而是要把一楼的叉车托盘（AXI burst）拆成三楼能接收的手推车规格（APB 单次访问），还要负责楼层之间的速度匹配、错误传递和排队管理。
