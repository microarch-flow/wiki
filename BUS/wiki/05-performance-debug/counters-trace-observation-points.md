...
BUS observability 的目标不是“抓到更多信号”，而是在问题发生后回答四个问题：谁发起了请求，在哪里排队，谁没有返回，错误由哪一层生成。Counter、trace 和观测点要围绕这四个问题设计。

如果 counter 语义不清，数字无法复盘；如果 trace 点没有触发条件，数据量会压垮调试；如果观测点只放在 master 端，就无法区分 master 没发、fabric 不收、slave 不回、return path 堵住。
...
Trace 适合记录低频但关键的事件链，不适合无限期抓全量 data。
...
trace 不能只靠手工打开。需要触发条件和 snapshot。
