...
评价一条高速公路，不能只问”今天跑了多少辆车”。**带宽**看单位时间通过多少车（吞吐量），**延迟**看一辆车从入口到出口等了多久（体验），**利用率**看路面忙不忙（资源状态），**拥塞**看哪个匝道在排队并把压力传回主路（因果定位）。

这四个指标必须一起看。高带宽可能伴随高尾延迟（今天通过了 10 万辆车，但有人堵了 3 小时）；高利用率可能来自有效载荷，也可能来自低效占路（路很忙，但都是抛锚车占着）；平均延迟正常时，少数关键流量仍可能被拖出长尾（大部分人 20 分钟到，但你被卡在收费站等了 2 小时）。
...
带宽是**结果**（今天跑了多少车），延迟是**体验**（你等了多久），利用率是**路况**（路面忙不忙），拥塞是**病因**（哪个路口堵了）。只看其中一个，就像只看"今天全市车流量正常"就说交通没问题——你堵在路上的 2 小时不会同意。
...
"平均通勤时间 30 分钟"听起来不错，但如果你有 5% 的概率堵 2 小时呢？延迟必须拆成多个阶段，否则无法定位"时间花在哪了"。
...
一条路"很忙"可能是因为车流量大（好事），也可能是因为有车抛锚占了两条车道（坏事）。利用率看资源忙了多久，但要分清**有效忙**和**无效占用**。
...
拥塞分析就像**交通事故调查**——你要找的是**第一辆追尾的车**，而不是排在最后面按喇叭的那辆。目标是找第一个让请求开始排队的 service point，而不是找最后一个看到 stall 的地方。
