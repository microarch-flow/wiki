...
打开 AXI 波形后，最容易浪费时间的做法是在 AW/W/B/AR/R 五条通道之间来回扫。AXI waveform debug 要先把问题分成读路径和写路径，再找第一处停止 forward progress 的握手，最后沿依赖链向上游和下游各扩一跳。

这页给出一个可执行顺序：分读写、找握手、闭环 transaction、看 ID/outstanding、解释 response、把波形翻译成系统语言。
...
AXI channel 的有效传输只在 `VALID && READY` 时发生。`VALID` 属于发送方，`READY` 属于接收方；单独看到一边为高，不代表 transaction 已发生。
...
波形结论不能停留在“READY 低”或“RVALID 没来”。要翻译成系统状态。
