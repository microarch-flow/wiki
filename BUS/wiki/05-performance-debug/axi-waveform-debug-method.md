...
但有一个边界必须先记住：  
`READY` 高、上游没发，不等于 sink 一定在阻塞；这也可能只是 source 当前没数据、没地址，或者被 outstanding 限制卡在更前面。AXI 里 `VALID` 属于发送方，`READY` 属于接收方，先看 `VALID&&READY` 真正发生了哪些握手，再判断谁在堵谁。
...
- 写事务通常要追到 `B` 返回才算闭环
- 读事务通常要追到最后一个 `R` beat（如 `RLAST`）才算闭环
