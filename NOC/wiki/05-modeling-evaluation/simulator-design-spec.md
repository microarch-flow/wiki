...
- workload-driven NoC architecture exploration
- flit-level
- wormhole
- credit-based flow control
- 可扩展到 VC / traffic class / hierarchical topology
...
- 哪些 link / port 最先饱和
- stall 主要来自 credit、switch、ejection 还是系统等待
- 不同拓扑、packet size、buffer depth、placement 的相对差异
- NoC 是否已经影响 tile utilization 和 workload completion time
...
- injection queue
- ejection queue
- traffic generator / consumer
- DMA / memory interface hooks
...
- deterministic routing 用函数生成 next hop
- source routing 用 route table 或 hop list
...
如果实现上需要，进一步拆成“读旧状态 / 写新状态”的双缓冲模式会更稳。
...
1. 单链路 credit 深度扫描
2. 2D mesh hotspot traffic
3. destination ejection 阻塞传播
4. flat mesh vs hierarchical NoC
5. GEMM-like 和 attention-like trace

## 第一版不必过度实现的功能

- 复杂 adaptive routing
- speculatively pipelined allocator
...
- 为什么这个 workload 慢
- 慢在网络哪里
- 该改 topology、buffer、QoS 还是 mapping
